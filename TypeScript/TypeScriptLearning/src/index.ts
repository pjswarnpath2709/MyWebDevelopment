...
const ans = func<string>("hello");

const func2 = <T, U>(n: T, m: U): { name: T; crank: U } => {
  return { name: n, crank: m };
};

const func3 = <T, U extends T>(n: T, m: U): { name: T; crank: U } => {
  return { name: n, crank: m };
};
...
interface PersonEx<T, U> extends Person<string, number> {
  name: T;
  age: U;
}

class Person2<T, U> {
  constructor(private prop1: T, private prop2: U) {}
}
